...
describe('CaseEvaluationPage - Role Gating and Read-Only Load', () => {
  ...
  it('enforces action visibility: RELATIONSHIP_MANAGER cannot see Run Assessment or sanction controls', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByText(/Sanctioning Authority Decision/i)).not.toBeInTheDocument();
    });
  });
  ...
  it('displays STALE_VERSION refresh guidance when conflict occurs', async () => {
    ...
    mockApiFetch.mockImplementation(async (url: string, opts?: RequestInit) => {
      if (opts?.method === 'POST') {
        return {
          ...
          data: { code: 'STALE_VERSION', message: 'Case version conflict. Refresh required.' },
        };
      }
      return {
        status: 200,
    ...
    await waitFor(() => {
      expect(screen.getByText(/STALE_VERSION: Case version conflict\. Refresh required\./i)).toBeInTheDocument();
  ...
  it('displays IDEMPOTENCY_IN_PROGRESS retry guidance when concurrent request occurs', async () => {
    ...
    mockApiFetch.mockImplementation(async (url: string, opts?: RequestInit) => {
      if (opts?.method === 'POST') {
        return {
          status: 409,
          error: 'IDEMPOTENCY_IN_PROGRESS: An identical request is currently being processed.',
          data: {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Please wait 5s and try again/i)).toBeInTheDocument();
    });
  });
});
