...
export default function LoginPage() {
  const [email, setEmail] = useState("credit@bank.example");
  const [password, setPassword] = useState("demo_dev_only_123");
    ...
    <div className="min-h-screen bg-navy-900 text-foreground flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="text-center mb-8 z-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-navy-800/80 border border-white/10 text-xs text-pulse-400 font-mono mb-3 shadow-lg">
          ...
          <span>IDBI INNOVATE 2026 • HACKATHON EDITION</span>
        </div>
        ...
        <p className="text-slate-400 text-sm sm:text-base max-w-md mx-auto">
          Autonomous Credit Assessment, BOLA Governance & Real-Time Decisioning Engine
        </p>
      </div>
      ...
      <div className="glass-panel w-full max-w-lg p-8 rounded-2xl shadow-2xl border border-white/10 z-10">
        <h2 className="text-xl font-semibold text-white mb-6 flex items-center justify-between">
          <span>Sign in to IDBI Core</span>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <span>Sign In to IDBI Vyapar Pulse</span>
        ...
        <div className="mt-6 pt-5 border-t border-white/5 text-center">
          <p className="text-xs text-slate-400">
            Protected by IDBI Core CAS & Role-Based BOLA Governance
          </p>
        </div>
      </div>

      {/* Footer Info */}
      <div className="mt-8 text-center text-xs text-slate-400 z-10 font-mono">
        IDBI Innovate 2026 • SME Credit Decisioning Platform • All Rights Reserved
      </div>
    </div>
  );
}
