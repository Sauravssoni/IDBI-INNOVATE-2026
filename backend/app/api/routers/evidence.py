...
def _build_metadata(r):
    return {
        ...
        "evidence_as_of": r.evidence_as_of.isoformat() if getattr(r, "evidence_as_of", None) else None,
...
def get_gst_evidence(case_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{"period": r.period_month.isoformat(), "declared_revenue": float(r.declared_revenue), "tax_paid": float(r.tax_paid), "status": "FILED", "metadata": _build_metadata(r)} for r in records]
...
def get_bank_evidence(case_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{"date": r.transaction_date.isoformat(), "amount": float(r.amount), "type": r.transaction_type, "category": r.category, "metadata": _build_metadata(r)} for r in records]
...
def get_invoice_evidence(case_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{"id": str(r.id), "date": r.invoice_date.isoformat(), "amount": float(r.amount), "status": r.status, "counterparty": r.counterparty_name, "metadata": _build_metadata(r)} for r in records]
...
def get_employment_evidence(case_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{"period": r.period_month.isoformat(), "employee_count": r.employee_count, "pf_remittance": float(r.total_pf_remittance), "metadata": _build_metadata(r)} for r in records]
...
def get_obligation_evidence(case_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{"id": str(r.id), "lender": "Unknown Lender", "facility_type": getattr(r, "facility_type", getattr(r, "loan_type", None)), "outstanding_balance": float(r.outstanding_balance), "monthly_emi": float(r.monthly_emi), "metadata": _build_metadata(r)} for r in records]
