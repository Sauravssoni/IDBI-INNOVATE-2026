...
def get_portfolio_metrics(
    ...
    db.query(Case).count()
    approved_cases = (
        db.query(Case).filter(Case.status == CaseStatus.HUMAN_APPROVED).count()
    )
    ...
    subq = (
        ...
        .group_by(AssessmentSnapshot.case_id)
        .subquery()
    )
    ...
    total_exposure = (
        sum(
            (
                a.canonical_assessment_json.get("assessment_range", {}).get(
                    "supportable_limit", 0
                )
                or 0
            )
    ...
    ecl_scenarios = {
        "base": {
            "ecl": ecl_base,
            "provision_ratio": (ecl_base / total_exposure * 100)
            if total_exposure
            else 0,
        },
        "adverse": {
            "ecl": ecl_base * 1.5,
            "provision_ratio": (ecl_base * 1.5 / total_exposure * 100)
            if total_exposure
            else 0,
        },
        "severe": {
            "ecl": ecl_base * 2.5,
            "provision_ratio": (ecl_base * 2.5 / total_exposure * 100)
            if total_exposure
            else 0,
        },
    }
    ...
    if avg_pd > 2.0:
        alerts.append(
            {
                "severity": "high",
                "message": f"Portfolio average PD is {avg_pd:.2f}%, exceeding 2.0% threshold.",
            }
        )
    if total_exposure > 500000000:
        alerts.append(
            {"severity": "medium", "message": "Total exposure is approaching limits."}
        )
