...
def seed_rangrez():
    ...
    existing_business = (
        db.query(Business)
        .filter(Business.business_id == "RANGREZ_TEXTILE_001")
        .first()
    )
    ...
    # 2. Create Business
    rangrez = Business(
        business_id="RANGREZ_TEXTILE_001",
    ...
    for m in range(18):
        ...
        db.add(
            BankTransaction(
                ...
                category="SALARY",
                source_system="ACCOUNT_AGGREGATOR",
                source_record_id=f"TXN-SAL-{m}-{rangrez.id}",
                ingestion_mode="SEEDED_PROTOTYPE",
                consent_id_fk=consents_by_source["ACCOUNT_AGGREGATOR"],
                data_connection_id_fk=connections_by_source["ACCOUNT_AGGREGATOR"],
            )
        ...
        for idx, amount in enumerate([inv1, inv2, inv3]):
            ...
            if m >= 16:
                ...
                inv = Invoice(
                    ...
                    source_record_id=f"INV-P-{m}-{idx}-{rangrez.id}",
                    ingestion_mode="SEEDED_PROTOTYPE",
                    consent_id_fk=consents_by_source["GST"],
                    data_connection_id_fk=connections_by_source["GST"],
                )
                ...
                inv = Invoice(
                    ...
                    status=status,
                    source_system="GST_E_INVOICE",
                    source_record_id=f"INV-D-{m}-{idx}-{rangrez.id}",
                    ingestion_mode="SEEDED_PROTOTYPE",
                    consent_id_fk=consents_by_source["GST"],
                    data_connection_id_fk=connections_by_source["GST"],
                )
                ...
                db.add(
                    InvoicePayment(
                        ...
                        amount=amount,
                        source_system="ACCOUNT_AGGREGATOR",
                        source_record_id=f"PAY-{m}-{idx}-{rangrez.id}",
                        ingestion_mode="SEEDED_PROTOTYPE",
                        consent_id_fk=consents_by_source["ACCOUNT_AGGREGATOR"],
    ...
    print(
        f"✅ Successfully seeded Shakti Precision Components (ID: {rangrez.id}) and its case."
    )
