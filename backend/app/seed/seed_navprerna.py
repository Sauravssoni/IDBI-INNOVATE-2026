...
def seed_navprerna():
    ...
    for m in range(18):
        ...
        db.add(
            BankTransaction(
                ...
                category="SALARY",
                source_system="ACCOUNT_AGGREGATOR",
                source_record_id=f"TXN-SAL-{m}-{navprerna.id}",
                ingestion_mode="SEEDED_PROTOTYPE",
                consent_id_fk=consents_by_source["ACCOUNT_AGGREGATOR"],
                data_connection_id_fk=connections_by_source["ACCOUNT_AGGREGATOR"],
            )
        ...
        for idx, amount in enumerate([inv1, inv2, inv3]):
            ...
            if m >= 16:
                ...
                inv = Invoice(
                    ...
                    source_record_id=f"INV-P-{m}-{idx}-{navprerna.id}",
                    ingestion_mode="SEEDED_PROTOTYPE",
                    consent_id_fk=consents_by_source["GST"],
                    data_connection_id_fk=connections_by_source["GST"],
                )
                ...
                inv = Invoice(
                    ...
                    status=status,
                    source_system="GST_E_INVOICE",
                    source_record_id=f"INV-D-{m}-{idx}-{navprerna.id}",
                    ingestion_mode="SEEDED_PROTOTYPE",
                    consent_id_fk=consents_by_source["GST"],
                    data_connection_id_fk=connections_by_source["GST"],
                )
                ...
                db.add(
                    InvoicePayment(
                        ...
                        amount=amount,
                        source_system="ACCOUNT_AGGREGATOR",
                        source_record_id=f"PAY-{m}-{idx}-{navprerna.id}",
                        ingestion_mode="SEEDED_PROTOTYPE",
                        consent_id_fk=consents_by_source["ACCOUNT_AGGREGATOR"],
    ...
    case = Case(
        ...
        requested_amount=Decimal("1500000.00"),
        currency="INR",
        status=CaseStatus.EVIDENCE_GATHERING,
    ...
    print(
        f"✅ Successfully seeded Shakti Precision Components (ID: {navprerna.id}) and its case."
    )
