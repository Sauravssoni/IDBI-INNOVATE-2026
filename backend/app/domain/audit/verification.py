...
def reconstruct_payload(event: AuditEvent) -> Dict[str, Any]:
    return {
        ...
        "idempotency_record_id": str(event.idempotency_record_id)
        if event.idempotency_record_id
        else None,
...
def verify_audit_chain(db: Session, case_id: str, current_user: User) -> dict:
    ...
    return {
        ...
        "analyst_event_status": "VERIFIED"
        if any(
            e.event_type
            in ("analyst_recommendation", "ANALYST_RECOMMENDATION", "DECISION_CREATED")
            for e in events
        )
        else "NOT VERIFIED",
        "human_decision_event_status": "VERIFIED"
        if any(e.event_type in ("human_decision", "SANCTION_DECISION") for e in events)
        else "NOT VERIFIED",
...
def _invalid_chain(
    last_valid_hash: str, reason: str, authorization_scope_valid: bool = True
) -> dict:
    return {
