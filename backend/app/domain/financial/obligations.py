...
def normalize_obligation_state(
    ...
    if state in {VERIFIED_OBLIGATIONS, "VERIFIED_MATCH", "VERIFIED_CIBIL_ONLY"}:
        ...
        if obligation_list:
            total = sum((decimal_or_none(o.get("monthly_emi")) or Decimal("0.00") for o in obligation_list), Decimal("0.00"))
            if total > 0:
        ...
        return UNKNOWN_OBLIGATIONS, Decimal("0.00"), ["VERIFIED_OBLIGATIONS requires positive authoritative debt-service amount."]
    ...
    if state in {VERIFIED_ZERO_DEBT, "VERIFIED_NO_DEBT"}:
        ...
        return UNKNOWN_OBLIGATIONS, Decimal("0.00"), ["VERIFIED_ZERO_DEBT requires explicit zero-debt evidence."]
    ...
    if state == "VERIFIED":
        ...
        if verified_zero_debt:
            return VERIFIED_ZERO_DEBT, Decimal("0.00"), []
        return UNKNOWN_OBLIGATIONS, Decimal("0.00"), ["Generic VERIFIED without debt amount or explicit zero-debt evidence is not assessable."]
    ...
    return UNKNOWN_OBLIGATIONS, Decimal("0.00"), [
