...
def compute_bankability_path(
    ...
    health_score = scores.get(
        "financial_health_index", scores.get("financial_health_score")
    )
    ...
    def run_sim(
        ...
        sim_decision = DecisionPolicy(
            sim_features, sim_scores, requested_amount, requested_product
        ).evaluate()
        sim_cap = FinancialCapacityEngine.compute_capacity_from_features(
            sim_features, requested_amount, requested_product
        )
        after_limit = Decimal(str(sim_decision.get("binding_limit", 0) or 0)).quantize(
        ...
        return {
            ...
            if not impact_known
            else f"FHI {health_score} -> {after_health}; Credit Health {scores.get('vyapar_credit_health_score')} -> {sim_scores.get('vyapar_credit_health_score')}",
            "expected_timeline_days": days,
            "target_state": sim_decision.get(
            ...
            "simulation_evidence": {
                ...
                "after_evidence_score": numeric(
                    sim_scores.get("evidence_confidence_score")
                ),
                ...
                "after_credit_health_score": sim_scores.get(
    ...
    if features.get("consent_status") != "VALID":
        ...
        milestones.append(
            run_sim(
                ...
                "consent_status",
                "Consent is valid and evidence rails are refreshed; no financial values are fabricated.",
                sim_features,
                30,
            )
        )
    ...
    if (
        base_cap.get("obligation_verification_state")
        not in ASSESSABLE_OBLIGATION_STATES
    ):
        milestones.append(
            {
                ...
                "projected_rate_bps": None,
                "prerequisites": [
                    "Authoritative obligation amount or explicit zero-debt evidence"
                ],
                "scenario_disclaimer": "Scenario only. Completion does not guarantee sanction.",
                "simulation_evidence": {
                    "before_evidence_score": float(evidence_score),
    ...
    if (
        current_state not in ("APPROVE", "READY_FOR_REVIEW")
        and inflows > 0
        and outflows > 0
    ):
        sim_features = copy.deepcopy(features)
        sim_bank = sim_features.setdefault("bank_metrics", {})
        sim_bank["operating_outflows_monthly"] = str(
            (outflows * Decimal("0.95")).quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            )
        )
        milestones.append(
            run_sim(
                ...
                "Scenario reduces only measured operating outflows by 5%.",
                sim_features,
                60,
            )
        )

    current_cycle = features.get("working_capital_metrics", {}).get(
        "operating_cycle_days", features.get("operating_cycle_days")
    )
    if (
        current_state not in ("APPROVE", "READY_FOR_REVIEW")
        ...
        sim_features = copy.deepcopy(features)
        sim_features.setdefault("working_capital_metrics", {})[
            "operating_cycle_days"
        ] = str(max(Decimal("30"), Decimal(str(current_cycle)) - Decimal("15")))
        milestones.append(
            run_sim(
                "MIL-004",
                "90_DAYS",
                ...
                "operating_cycle_days",
                "Scenario reduces only the measured operating cycle, floored at 30 days.",
                sim_features,
                90,
            )
        )
    ...
    tgt_req = (
        float(target_amount) if target_amount is not None else float(requested_amount)
    )
    curr_lim_flt = float(
        current_limit.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    )
    gap = max(0.0, tgt_req - curr_lim_flt)
    max_achievable = max(
        [m.get("projected_limit_inr", 0.0) for m in milestones] + [curr_lim_flt]
    )
    ...
    hindi_presentation = {
        ...
        "milestone_actions": [
            m.get("hindi_action", m.get("action", "")) for m in milestones
        ],
    }
...
def simulate_bankability_variable(
    ...
    try:
        ...
        base_cap = {
            "supportable_limit_inr": str(base_decision.get("binding_limit", 0)),
            "verified_dscr": str(features.get("bank_metrics", {}).get("dscr", "1.0")),
        }
    ...
    if (
        "operating_inflows_monthly" in overrides
        and overrides["operating_inflows_monthly"] is not None
    ):
        bank_sim["operating_inflows_monthly"] = str(
            overrides["operating_inflows_monthly"]
        )
    if (
        "operating_outflows_monthly" in overrides
        and overrides["operating_outflows_monthly"] is not None
    ):
        bank_sim["operating_outflows_monthly"] = str(
            overrides["operating_outflows_monthly"]
        )
    ...
    if (
        "operating_cycle_days" in overrides
        and overrides["operating_cycle_days"] is not None
    ):
    ...
    if (
        "verified_existing_debt_service_monthly" in overrides
        and overrides["verified_existing_debt_service_monthly"] is not None
    ):
        sim_features["verified_existing_debt_service_monthly"] = str(
            overrides["verified_existing_debt_service_monthly"]
        )
    ...
    if (
        "obligation_verification_state" in overrides
        and overrides["obligation_verification_state"] is not None
    ):
        sim_features["obligation_verification_state"] = str(
            overrides["obligation_verification_state"]
        )
    ...
    try:
        ...
        sim_cap = {
            "supportable_limit_inr": str(sim_decision.get("binding_limit", 0)),
            "verified_dscr": str(bank_sim.get("dscr", "1.0")),
        }

    before_limit = float(
        Decimal(str(base_decision.get("binding_limit", 0))).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
    )
    after_limit = float(
        Decimal(str(sim_decision.get("binding_limit", 0))).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
    )
    ...
    try:
        ...
        before_dscr = (
            None if base_dscr_value is None else float(Decimal(str(base_dscr_value)))
        )
    except Exception:
        before_dscr = None
    try:
        ...
        after_dscr = (
            None if sim_dscr_value is None else float(Decimal(str(sim_dscr_value)))
        )
    except Exception:
        after_dscr = None
    ...
    return {
        ...
        "uplift_summary": {
            ...
            "health_score_uplift": None
            if after_health is None or before_health is None
            else after_health - before_health,
            "fhi_uplift": None
            if after_fhi is None or before_fhi is None
            else after_fhi - before_fhi,
            "credit_score_uplift": None
            if after_credit is None or before_credit is None
            else after_credit - before_credit,
            "dscr_uplift": None
            if after_dscr is None or before_dscr is None
            else after_dscr - before_dscr,
        },
