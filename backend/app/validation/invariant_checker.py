...
from app.api.routers.cases import _hash_package_data
    ...
    profiles: List[tuple[str, Dict[str, Any]]] = [
        ...
        (
            "thin_file_assessable",
            {
                ...
                "bank_metrics": {
                    **base_features["bank_metrics"],
                    "operating_inflows_monthly": "2000000",
                    "operating_outflows_monthly": "1800000",
                },
                "monthly_revenue_inr": "2000000",
            },
        ),
        ("insufficient_evidence", {"monthly_revenue_inr": "", "bank_metrics": {}}),
        ("unknown_obligations", {"obligation_verification_state": "UNKNOWN"}),
        (
            "verified_zero_debt",
            {
                ...
                "bank_metrics": {
                    **base_features["bank_metrics"],
                    "verified_debt_service_monthly": "0",
                },
        ...
        (
            "negative_cash",
            {
                "bank_metrics": {
                    **base_features["bank_metrics"],
                    "operating_inflows_monthly": "3000000",
                    "operating_outflows_monthly": "3500000",
                }
            },
        ),
        (
...
def run_validation_suite() -> Dict[str, Any]:
    ...
    for idx, features in enumerate(cases):
        ...
        try:
            ...
            inflows = Decimal(
                features.get("bank_metrics", {}).get("operating_inflows_monthly", "0")
            )
            outflows = Decimal(
                features.get("bank_metrics", {}).get("operating_outflows_monthly", "0")
            )
            existing_ds = Decimal(
                features.get("bank_metrics", {}).get(
                    "verified_debt_service_monthly", "0"
                )
            )
            ...
            cycle = Decimal(
                str(
                    features.get("working_capital_metrics", {}).get(
            ...
            eligible = Decimal(
                features.get("invoice_metrics", {}).get("eligible_amount", "0")
            )
            ...
            if obs_state == "UNKNOWN":
                if any(lim > 0 for lim in limit_dict.values()):
            ...
            if (
                features.get("monthly_revenue_inr")
                and obs_state != "UNKNOWN"
                and operating_cash > 0
            ):
                features_higher_debt = json.loads(json.dumps(features))
                features_higher_debt["bank_metrics"][
                    "verified_debt_service_monthly"
                ] = str(existing_ds + 100000)
                features_higher_debt["verified_existing_debt_service_monthly"] = str(
                    existing_ds + 100000
                )
                ...
                wc_limit_hd = {
                    L["method"]: Decimal(str(L["calculated_limit"])) for L in limits_hd
                }.get("WORKING_CAPITAL_LINE", Decimal("0"))
            ...
            if features.get("monthly_revenue_inr") and obs_state != "UNKNOWN":
                ...
                features_lc["bank_metrics"]["operating_inflows_monthly"] = str(
                    max(10000, inflows - 1000000)
                )
                ...
                wc_limit_lc = {
                    L["method"]: Decimal(str(L["calculated_limit"])) for L in limits_lc
                }.get("WORKING_CAPITAL_LINE", Decimal("0"))
            ...
            if features.get("monthly_revenue_inr") and obs_state != "UNKNOWN":
                ...
                rec_limit_hc = {
                    L["method"]: Decimal(str(L["calculated_limit"])) for L in limits_hc
                }.get("RECEIVABLES_FINANCE", Decimal("0"))
                if rec_limit_hc > rec_limit:
                    raise ValueError(
                        "Higher concentration improved receivables capacity"
                    )
            ...
            results["failures"].append(
                {"case_index": idx, "profile": profile, "error": str(e)}
            )
        ...
        results["failures"].append(
            {
                "case_index": -1,
                "profile": "all",
                "error": "Same seed did not produce same features",
            }
        )
        results["invariants_failed"] += 1
    else:
    ...
    for i in range(min(25, len(cases))):
        ...
        mismatches = []
        if json.dumps(canonical_scores, sort_keys=True, default=str) != json.dumps(
            replay_scores, sort_keys=True, default=str
        ):
            mismatches.append("scores")
        if json.dumps(canonical_limits, sort_keys=True, default=str) != json.dumps(
            replay_limits, sort_keys=True, default=str
        ):
