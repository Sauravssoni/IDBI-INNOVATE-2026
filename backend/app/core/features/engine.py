...
class FeatureEngine:
    ...
    def _derive_gst_metrics(self) -> Dict[str, Any]:
        gst_records = self.db.query(GSTPeriod).filter(GSTPeriod.business_id_fk == self.business_id).all()
        if not gst_records:
            return {"months_filed": 0, "avg_monthly_revenue": "0.00", "revenue_cv": "0.0000", "trend": "INSUFFICIENT_DATA"}
        ...
        trend = "STABLE"
        if len(revenues) >= 12:
            sorted_revenues = [r.declared_revenue for r in sorted(gst_records, key=lambda x: x.period_month)]
        ...
        return {
            ...
            "avg_monthly_revenue": str(avg_revenue.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)),
            "revenue_cv": str(cv.quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP)),
            "trend": trend
        }

    def _derive_bank_metrics(self) -> Dict[str, Any]:
        bank_txns = self.db.query(BankTransaction).filter(BankTransaction.business_id_fk == self.business_id).all()
        if not bank_txns:
            return {"total_credits": "0.00", "total_debits": "0.00", "avg_monthly_credits": "0.00", "avg_monthly_debits": "0.00"}
            
        credits = sum((t.amount for t in bank_txns if t.transaction_type == "CREDIT"), Decimal("0"))
        debits = sum((t.amount for t in bank_txns if t.transaction_type == "DEBIT"), Decimal("0"))
        
        # Assumes 18 months based on standard pulling
        months = Decimal("18.0")
        
        return {
            "total_credits": str(credits.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)),
            "total_debits": str(debits.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)),
            "avg_monthly_credits": str((credits / months).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)),
            "avg_monthly_debits": str((debits / months).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
        }
    ...
    def _derive_reconciliation_metrics(self) -> Dict[str, Any]:
        ...
        total_gst_revenue = Decimal(gst["avg_monthly_revenue"]) * Decimal(str(months_filed))
        
        ratio = Decimal(bank["total_credits"]) / total_gst_revenue if total_gst_revenue > 0 else Decimal("0")
        ...
        return {
            "gst_bank_ratio": str(ratio.quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP)),
            "status": status
        }

    def _derive_employment_metrics(self) -> Dict[str, Any]:
        epfo_records = self.db.query(EmploymentPeriod).filter(EmploymentPeriod.business_id_fk == self.business_id).all()
        if not epfo_records:
    ...
    def _derive_receivable_metrics(self) -> Dict[str, Any]:
        invoices = self.db.query(Invoice).filter(Invoice.business_id_fk == self.business_id).all()
        if not invoices:
            return {"total_invoices": 0, "overdue_ratio": "0.00", "concentration_risk": "UNKNOWN", "eligible_amount": "0.00"}
        ...
        eligible = sum((i.amount for i in invoices if i.status == "PENDING"), Decimal("0"))
        
        return {
            "total_invoices": len(invoices),
            "top_buyer_concentration": str(top_buyer_share.quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP)),
            "concentration_risk": concentration,
            "avg_payment_delay_days": str(avg_delay),
            "eligible_amount": str(eligible.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
        }
