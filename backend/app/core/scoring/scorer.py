...
class ScoringEngine:
    ...
    def compute_all_scores(self) -> Dict[str, Any]:
        ...
        if (
            not self.features.get("monthly_revenue_inr")
            or Decimal(str(self.features.get("monthly_revenue_inr", "0"))) <= 0
        ):
        ...
        ledger.append(
            {
                ...
                "raw_points": float(base),
                "weighted_points": float(base),
                "direction": "BASE",
            ...
            ledger.append(
                {
                    "rule_id": "SC-002",
                    "pillar": "Liquidity",
                    "input_name": "Operating Margin",
                    "input_value": f"{margin * 100:.1f}%",
                    "threshold_or_band": ">10%",
                    "raw_points": float(liq_pts),
                    "weighted_points": float(liq_pts),
                    "direction": "EARNED",
                    "reason_code": "LIQ_MARGIN",
                    "technical_explanation": "Operating margin points",
                    "applicant_explanation_en": "Healthy cash buffer.",
                    "applicant_explanation_hi": "स्वस्थ नकदी बफर।",
                    "evidence_ids": inflow_evd + outflow_evd,
                    "running_score": float(base + earned + deductions),
                }
            )
            liq_status, liq_missing = "VERIFIED", []
        else:
            liq_pts, liq_status, liq_missing = (
                None,
                "MISSING_DATA",
        ...
        existing_dscr = None
        if (
            obligation_state in ASSESSABLE_OBLIGATION_STATES
            ...
            cf_pts = (
                Decimal("150")
                if existing_dscr >= Decimal("2.0")
                else Decimal("100")
                if existing_dscr >= Decimal("1.5")
                else Decimal("50")
                if existing_dscr >= Decimal("1.1")
                else Decimal("0")
            )
            earned += cf_pts
            ledger.append(
                {
                    ...
                    "input_name": "DSCR",
                    "input_value": f"{existing_dscr:.2f}x",
                    "threshold_or_band": ">1.5x",
                    "raw_points": float(cf_pts),
                    "weighted_points": float(cf_pts),
                    "direction": "EARNED",
                    "reason_code": "CF_DSCR",
                    "technical_explanation": "DSCR points",
                    "applicant_explanation_en": "Strong debt coverage.",
                    "applicant_explanation_hi": "मजबूत ऋण कवरेज।",
                    "evidence_ids": inflow_evd + outflow_evd + obligation_evd,
                    "running_score": float(base + earned + deductions),
                }
            )
            cf_status, cf_missing = "VERIFIED", []
        else:
            cf_pts, cf_status, cf_missing = (
                None,
                "MISSING_DATA",
        ...
        if months >= 6 and cv_val != "UNKNOWN":
            ...
            ledger.append(
                {
                    "rule_id": "SC-004",
                    "pillar": "Revenue Stability",
                    "input_name": "Revenue CV",
                    "input_value": f"{cv * 100:.1f}%",
                    "threshold_or_band": "<30%",
                    "raw_points": float(rev_pts),
                    "weighted_points": float(rev_pts),
                    "direction": "EARNED",
                    "reason_code": "REV_STAB",
                    "technical_explanation": "Revenue CV points",
                    "applicant_explanation_en": "Stable revenue.",
                    "applicant_explanation_hi": "स्थिर राजस्व।",
                    "evidence_ids": gst_evd,
                    "running_score": float(base + earned + deductions),
                }
            )
            rev_status, rev_missing = "VERIFIED", []
        else:
            rev_pts, rev_status, rev_missing = (
                None,
                "MISSING_DATA",
                ["six_month_revenue_series"],
            )
        ...
        if (
            ...
            ledger.append(
                {
                    "rule_id": "SC-005",
                    "pillar": "Repayment Burden",
                    "input_name": "Existing Burden",
                    "input_value": f"{burden * 100:.1f}%",
                    "threshold_or_band": "<45%",
                    "raw_points": float(rep_pts),
                    "weighted_points": float(rep_pts),
                    "direction": "EARNED",
                    "reason_code": "REP_BURD",
                    "technical_explanation": "Repayment burden points",
                    "applicant_explanation_en": "Low existing debt.",
                    "applicant_explanation_hi": "कम मौजूदा कर्ज।",
                    "evidence_ids": obligation_evd + inflow_evd,
                    "running_score": float(base + earned + deductions),
                }
            )
            rep_status, rep_missing = "VERIFIED", []
        else:
            rep_pts, rep_status, rep_missing = (
                None,
                "MISSING_DATA",
                ["verified_existing_debt_service_monthly"],
            )
            ...
            ledger.append(
                {
                    "rule_id": "SC-006",
                    "pillar": "Compliance",
                    "input_name": "GST/Bank Ratio",
                    "input_value": f"{ratio:.2f}",
                    "threshold_or_band": "0.9-1.1",
                    "raw_points": float(comp_pts),
                    "weighted_points": float(comp_pts),
                    "direction": "EARNED",
                    "reason_code": "COMP_RECON",
                    "technical_explanation": "GST vs Bank reconciliation",
                    "applicant_explanation_en": "Strong compliance.",
                    "applicant_explanation_hi": "मजबूत अनुपालन।",
                    "evidence_ids": gst_evd + inflow_evd,
                    "running_score": float(base + earned + deductions),
                }
            )
            comp_status, comp_missing = "VERIFIED", []
        else:
            comp_pts, comp_status, comp_missing = (
                None,
                "MISSING_DATA",
        ...
        if conc_val != "UNKNOWN":
            ...
            res_pts = (
                Decimal("60")
                if conc <= Decimal("0.25")
                else Decimal("30")
                if conc <= Decimal("0.40")
                else Decimal("0")
            )
            earned += res_pts
            ledger.append(
                {
                    "rule_id": "SC-007",
                    "pillar": "Resilience",
                    "input_name": "Buyer Concentration",
                    "input_value": f"{conc * 100:.1f}%",
                    "threshold_or_band": "<25%",
                    "raw_points": float(res_pts),
                    "weighted_points": float(res_pts),
                    "direction": "EARNED",
                    "reason_code": "RES_CONC",
                    "technical_explanation": "Buyer concentration points",
                    "applicant_explanation_en": "Diversified buyers.",
                    "applicant_explanation_hi": "विविध खरीदार।",
                    "evidence_ids": gst_evd,
                    "running_score": float(base + earned + deductions),
                }
            )
            ...
            res_pts, res_status, res_missing = (
                None,
                "MISSING_DATA",
        ...
        assessable = not missing_core and all(
            p is not None for p in [liq_pts, cf_pts, rev_pts, rep_pts]
        )
        ...
        pillar_items = {
            ...
            "cash_flow_capacity": pillar(
                "Cash Flow Capacity", cf_pts, 150, cf_status, cf_missing
            ),
            "revenue_stability_momentum": pillar(
                "Revenue Stability", rev_pts, 90, rev_status, rev_missing
            ),
            "repayment_burden_discipline": pillar(
                "Repayment Burden", rep_pts, 120, rep_status, rep_missing
            ),
            "compliance_formalisation": pillar(
                "Compliance", comp_pts, 60, comp_status, comp_missing
            ),
            "concentration_resilience": pillar(
                "Concentration", res_pts, 60, res_status, res_missing
            ),
        }

        if assessable:
            final_score_dec = base + earned - deductions
            vyapar_credit_health_score = int(final_score_dec)
        ...
        return {
            ...
            "earned_contributions": float(earned),
            "deductions": float(deductions),
            "final_score": vyapar_credit_health_score,
