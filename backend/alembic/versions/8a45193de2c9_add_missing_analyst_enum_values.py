...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TYPE analystrecommendationaction ADD VALUE IF NOT EXISTS 'RECOMMEND_AS_REQUESTED'"
    )
    op.execute(
        "ALTER TYPE analystrecommendationaction ADD VALUE IF NOT EXISTS 'RECOMMEND_DECLINE'"
    )
