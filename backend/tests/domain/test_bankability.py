...
from app.domain.bankability.path import (
    compute_bankability_path,
    simulate_bankability_variable,
)
...
def test_compute_bankability_path_deterministic_milestones():
    features = {
        ...
        "working_capital_metrics": {"operating_cycle_days": 75},
...
def test_compute_bankability_path_already_approved():
    features = {
        ...
        "reconciliation_metrics": {"gst_bank_ratio": "1.02"},
    }
    scores = {
    ...
    path = compute_bankability_path(
        features, scores, Decimal("2500000"), "WORKING_CAPITAL_LINE"
    )
...
def test_simulate_bankability_variable():
    features = {
        ...
        "reconciliation_metrics": {"gst_bank_ratio": "0.85"},
    }
    scores = {
        "evidence_confidence_score": Decimal("60.0"),
        "financial_health_score": Decimal("65.0"),
    ...
    result = simulate_bankability_variable(
        features, scores, Decimal("5000000"), "WORKING_CAPITAL_LINE", overrides
    )
    ...
    if (
        before["financial_health_score"] is not None
        and after["financial_health_score"] is not None
    ):
...
def test_bankability_path_does_not_fabricate_requested_amount_targets():
    ...
    scores = {
        "evidence_confidence_score": Decimal("0.0"),
        "financial_health_score": None,
    }

    path = compute_bankability_path(
        features, scores, Decimal("5000000"), "WORKING_CAPITAL_LINE"
    )
    ...
    assert all(
        m["impact_on_score"] == "IMPACT_NOT_QUANTIFIABLE" for m in path["milestones"]
    )
