...
def test_get_gst_evidence(client, case_id):
    ...
    response = client.get(
        f"/api/cases/{case_id}/evidence/gst", headers=ca_auth["headers"]
    )
    assert response.status_code == 200
    data = response.json()
...
def test_get_bank_evidence(client, case_id):
    ...
    client.cookies.update(ca_auth["cookies"])
    response = client.get(
        f"/api/cases/{case_id}/evidence/bank", headers=ca_auth["headers"]
    )
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_get_invoice_evidence(client, case_id):
    ...
    client.cookies.update(ca_auth["cookies"])
    response = client.get(
        f"/api/cases/{case_id}/evidence/invoices", headers=ca_auth["headers"]
    )
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_get_employment_evidence(client, case_id):
    ...
    client.cookies.update(ca_auth["cookies"])
    response = client.get(
        f"/api/cases/{case_id}/evidence/employment", headers=ca_auth["headers"]
    )
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_get_obligation_evidence(client, case_id):
    ...
    client.cookies.update(ca_auth["cookies"])
    response = client.get(
        f"/api/cases/{case_id}/evidence/obligations", headers=ca_auth["headers"]
    )
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_get_credit_twin_no_hardcodes(client, case_id):
    ...
    client.cookies.update(ca_auth["cookies"])
    response = client.get(
        f"/api/cases/{case_id}/credit-twin", headers=ca_auth["headers"]
    )
    assert response.status_code == 200
    data = response.json()
...
def test_get_reconciliation_missing_evidence(client, case_id):
    ...
    client.cookies.update(ca_auth["cookies"])
    response = client.get(
        f"/api/cases/{case_id}/reconciliation", headers=ca_auth["headers"]
    )
    assert response.status_code == 200
    data = response.json()
    ...
    assert "GST_BANK_RECON" in check_ids
    assert "INVOICE_PAYMENT_RECON" in check_ids
    assert "PAYROLL_EMPLOYMENT_RECON" in check_ids
    assert "OBLIGATION_DEBT_SERVICE_RECON" in check_ids
    assert "CIRCULAR_FLOW_EVIDENCE" in check_ids

    invoice_check = next(
        c for c in data["checks"] if c["check_id"] == "INVOICE_PAYMENT_RECON"
    )
    assert invoice_check["status"] != "MISSING_EVIDENCE"

    payroll_check = next(
        c for c in data["checks"] if c["check_id"] == "PAYROLL_EMPLOYMENT_RECON"
    )
    assert payroll_check["status"] != "MISSING_EVIDENCE"

    circular_check = next(
        (c for c in data["checks"] if c["check_id"] == "CIRCULAR_FLOW_EVIDENCE"), None
    )
...
def test_bola_cross_branch_denial_evidence(client, case_id, db):
    ...
    pw = os.environ.get("DEMO_USER_PASSWORD")
    if not user:
        user = User(
            id=str(uuid.uuid4()),
            email=email,
    ...
    response = client.get(
        f"/api/cases/{case_id}/evidence/gst", headers=rm_auth["headers"]
    )
...
def test_bola_system_admin_denial_evidence(client, case_id):
    ...
    response = client.get(
        f"/api/cases/{case_id}/evidence/gst", headers=sa_auth["headers"]
    )
    assert response.status_code in [403, 404]
