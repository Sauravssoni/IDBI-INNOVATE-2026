...
def test_localhost_url_rejected_in_production(clean_env):
    ...
    import app.db.session as session_module
    with pytest.raises(RuntimeError, match="Production DATABASE_URL must point to managed PostgreSQL"):
        importlib.reload(session_module)
...
def test_migration_behind_readiness(mock_session_local, monkeypatch):
    ...
    class DummyScript:
        def get_heads(self):
            return ["new_version"]
    monkeypatch.setattr(main_module.ScriptDirectory, "from_config", lambda cfg: DummyScript())
