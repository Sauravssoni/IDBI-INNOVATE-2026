...
def deterministic_data(db: Session):
    ...
    db.execute(text("INSERT INTO regions (id, code, name, created_at, updated_at) VALUES (:id, 'REG-AUDIT', 'Audit Region', :ct, :ct)"), {"id": reg_id, "ct": utc_now()})
    
    branch_a_id = str(uuid.uuid4())
    branch_b_id = str(uuid.uuid4())
    db.execute(text("INSERT INTO branches (id, region_id, code, name, created_at, updated_at) VALUES (:id, :reg_id, 'BR-A', 'Branch A', :ct, :ct)"), {"id": branch_a_id, "reg_id": reg_id, "ct": utc_now()})
    db.execute(text("INSERT INTO branches (id, region_id, code, name, created_at, updated_at) VALUES (:id, :reg_id, 'BR-B', 'Branch B', :ct, :ct)"), {"id": branch_b_id, "reg_id": reg_id, "ct": utc_now()})
    ...
    for email, (name, role, branch_id, scope_role) in users.items():
        ...
        user_ids[email] = uid
        db.execute(
            text("INSERT INTO users (id, email, hashed_password, full_name, role, is_active, created_at, updated_at) VALUES (:id, :email, :pw_hash, :name, :role, true, :ct, :ct)"),
            {"id": uid, "email": email, "pw_hash": pw_hash, "name": name, "role": role, "ct": utc_now()}
        )
        if branch_id and scope_role:
            db.execute(
                text("INSERT INTO user_branch_scopes (id, user_id, branch_id, scope_role, active, can_read, can_recommend, created_at, updated_at) VALUES (:id, :uid, :bid, :srole, true, true, true, :ct, :ct)"),
                {"id": str(uuid.uuid4()), "uid": uid, "bid": branch_id, "srole": scope_role, "ct": utc_now()}
            )

    # 3. Business & Cases
    biz_id = str(uuid.uuid4())
    db.execute(text("INSERT INTO businesss (id, business_id, legal_name, sector, created_at, updated_at) VALUES (:id, 'BIZ-AUDIT', 'Audit Biz', 'Retail', :ct, :ct)"), {"id": biz_id, "ct": utc_now()})

    case_a_id = str(uuid.uuid4())
    case_b_id = str(uuid.uuid4())
    db.execute(
        text("INSERT INTO cases (id, business_id_fk, status, requested_amount, requested_product, originating_branch_id, assigned_relationship_manager_id, currency, version, created_at, updated_at) VALUES (:id, :bid, 'INITIATED', 100, 'WORKING_CAPITAL_LINE', :branch, :rm, 'INR', 1, :ct, :ct)"),
        {"id": case_a_id, "bid": biz_id, "branch": branch_a_id, "rm": user_ids["rm_a@bank.example"], "ct": utc_now()}
    )
    db.execute(
        text("INSERT INTO cases (id, business_id_fk, status, requested_amount, requested_product, originating_branch_id, assigned_relationship_manager_id, currency, version, created_at, updated_at) VALUES (:id, :bid, 'INITIATED', 200, 'WORKING_CAPITAL_LINE', :branch, :rm, 'INR', 1, :ct, :ct)"),
        {"id": case_b_id, "bid": biz_id, "branch": branch_b_id, "rm": user_ids["rm_b@bank.example"], "ct": utc_now()}
    )
    ...
    for i in range(1, 4):
        db.execute(
            text("INSERT INTO audit_events (id, case_id, event_sequence, event_type, actor, actor_role, audit_schema_version, hash_algorithm, created_at, updated_at) VALUES (:id, :case, :seq, 'EVALUATE', :actor, 'RELATIONSHIP_MANAGER', 1, 'sha256', :ct, :ct)"),
            {"id": str(uuid.uuid4()), "case": case_a_id, "seq": i, "actor": user_ids["rm_a@bank.example"], "ct": utc_now() + timedelta(minutes=i)}
        )

    # Case B: 1 event
    db.execute(
        text("INSERT INTO audit_events (id, case_id, event_sequence, event_type, actor, actor_role, audit_schema_version, hash_algorithm, created_at, updated_at) VALUES (:id, :case, 1, 'EVALUATE', :actor, 'RELATIONSHIP_MANAGER', 1, 'sha256', :ct, :ct)"),
        {"id": str(uuid.uuid4()), "case": case_b_id, "actor": user_ids["rm_b@bank.example"], "ct": utc_now()}
    )
    ...
    # Teardown
    db.execute(text("DELETE FROM audit_events WHERE case_id IN (:ca, :cb)"), {"ca": case_a_id, "cb": case_b_id})
    db.execute(text("DELETE FROM cases WHERE id IN (:ca, :cb)"), {"ca": case_a_id, "cb": case_b_id})
    ...
    for uid in user_ids.values():
        ...
        db.execute(text("DELETE FROM user_branch_scopes WHERE user_id = :uid"), {"uid": uid})
        db.execute(text("DELETE FROM users WHERE id = :uid"), {"uid": uid})
    db.execute(text("DELETE FROM branches WHERE id IN (:ba, :bb)"), {"ba": branch_a_id, "bb": branch_b_id})
...
def test_auditor_branch_isolation(client: TestClient, db: Session, deterministic_data):
    ...
    b_res = client.get(f"/api/cases/{deterministic_data['case_b']}/audit", headers=aud_a_auth["headers"])
...
def test_rm_unassigned_case_denial(client: TestClient, db: Session, deterministic_data):
    ...
    res = client.get(f"/api/cases/{deterministic_data['case_b']}/audit", headers=rm_a_auth["headers"])
    assert res.status_code == 404

def test_risk_admin_scope_isolation(client: TestClient, db: Session, deterministic_data):
...
def test_system_admin_exclusion(client: TestClient, db: Session, deterministic_data):
    ...
    case_res = client.get(f"/api/cases/{deterministic_data['case_a']}/audit", headers=sys_auth["headers"])
    assert case_res.status_code == 404

def test_audit_pagination_and_ordering(client: TestClient, db: Session, deterministic_data):
    aud_a_auth = get_auth_headers(client, "auditor_a@bank.example")
    client.cookies.clear()
    client.cookies.update(aud_a_auth["cookies"])
    ...
    res_offset = client.get("/api/audit/logs?limit=2&offset=2", headers=aud_a_auth["headers"])
...
def test_audit_portfolio_exact_response_keys(client: TestClient, db: Session, deterministic_data):
    aud_a_auth = get_auth_headers(client, "auditor_a@bank.example")
    client.cookies.clear()
    client.cookies.update(aud_a_auth["cookies"])
