...
def setup_bola_data(db_session):
    ...
    biz1 = Business(
        business_id=f"BIZ1_{unique_suffix}", legal_name="Biz 1", sector="Retail"
    )
    biz2 = Business(
        business_id=f"BIZ2_{unique_suffix}", legal_name="Biz 2", sector="Retail"
    )
    db_session.add(biz1)
    db_session.add(biz2)
    db_session.flush()
...
def test_bola_system_admin_has_no_case_access(setup_bola_data):
    login_resp = login(
        setup_bola_data["users"][UserRole.SYSTEM_ADMIN].email, "securepass123"
    )
    cookies = {
        "vyapar_session_token": get_cookie_from_response(
            login_resp, "vyapar_session_token"
        )
    }
...
def test_bola_rm_sees_only_assigned_cases(setup_bola_data):
    login_resp = login(
        setup_bola_data["users"][UserRole.RELATIONSHIP_MANAGER].email, "securepass123"
    )
    cookies = {
        "vyapar_session_token": get_cookie_from_response(
            login_resp, "vyapar_session_token"
...
def test_bola_risk_admin_sees_scoped_branch_cases(setup_bola_data):
    login_resp = login(
        setup_bola_data["users"][UserRole.RISK_ADMIN].email, "securepass123"
    )
    cookies = {
        "vyapar_session_token": get_cookie_from_response(
            login_resp, "vyapar_session_token"
