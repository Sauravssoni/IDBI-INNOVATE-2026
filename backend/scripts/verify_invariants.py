...
def verify_invariants():
    ...
    for case in cases:
        ...
        try:
            ...
            assert final_amt <= req_amt, (
                f"Invariant failed: Final amount {final_amt} > Requested {req_amt}"
            )
            ...
            binding_stage = next(
                (
                    s
                    for s in bridge["stages"]
                    if s["stage_id"] == bridge["binding_constraint"]
                ),
                None,
            )
            if binding_stage:
                assert Decimal(str(binding_stage["calculated_value"])) == final_amt, (
                    "Invariant failed: Binding constraint mismatch"
                )

            print(
                f"[PASS] {cid} - Requested: {req_amt}, Final: {final_amt}, Binding: {bridge['binding_constraint']}"
            )
            passed += 1
