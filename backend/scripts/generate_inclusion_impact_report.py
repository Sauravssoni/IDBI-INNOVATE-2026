...
def generate_cohort(cohort_size: int = 500) -> List[Dict[str, Any]]:
    ...
    for i in range(1, cohort_size + 1):
        ...
        inflows_monthly = (base_turnover / Decimal("12")).quantize(
        ...
        debits_monthly = (inflows_monthly * Decimal("0.75")).quantize(
            Decimal("1"), rounding=ROUND_HALF_UP
        )
        ...
        if dscr > Decimal("0") and (i % 5 != 0):
            verified_ds = (operating_cash_available / dscr).quantize(
                Decimal("1"), rounding=ROUND_HALF_UP
            )
        else:
        ...
        has_digital_commerce = ((i % 2) == 0) or seg_name in (
            "Retail Trade & Kirana",
            "Textile & Garments",
        )
        ...
        requested_amount = (inflows_monthly * Decimal("4")).quantize(
            Decimal("1"), rounding=ROUND_HALF_UP
        )
...
def evaluate_cohort(cohort: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    demographic_impact = {
        ...
        "new_to_credit_or_thin_file": {
            "total": 0,
            "included": 0,
            "unlocked_inr": Decimal("0"),
        },
    ...
    for item in cohort:
        ...
        limit_inr = Decimal(
            str(
                capacity.get(
                    "binding_product_limit",
                    capacity.get("supportable_limit_inr", Decimal("0")),
                )
            )
        )
        ...
        is_traditionally_rejected = (
            dscr < Decimal("1.33")
            or is_ntc
            or features["obligation_verification_state"] != "VERIFIED"
        )
        ...
        if is_traditionally_rejected:
            ...
            if dec_value in (
                ...
                SystemRecommendation.ENHANCED_DUE_DILIGENCE.value,
            ):
        ...
        if tier not in tier_breakdown:
            tier_breakdown[tier] = {
                "total": 0,
                "ready_or_conditional": 0,
                "bankability_path": 0,
                "total_limit_inr": Decimal("0"),
            }
    ...
    # Final formatting
    avg_fhi = float(
        (total_fhi / Decimal(str(total_evaluated))).quantize(
    ...
    inclusion_rate_pct = (
        float(
            (
                Decimal(vyapar_pulse_included_count)
                / Decimal(traditional_rejected_or_unassessed_count)
                * Decimal("100")
            ).quantize(Decimal("0.1"), rounding=ROUND_HALF_UP)
        )
        if traditional_rejected_or_unassessed_count > 0
        else 100.0
    )
    ...
    return {
        ...
        "demographic_inclusion_impact": {
            k: {
                ...
                "inclusion_pct": float(
                    (
                        Decimal(v["included"]) / Decimal(v["total"]) * Decimal("100")
                    ).quantize(Decimal("0.1"), rounding=ROUND_HALF_UP)
                )
                if v["total"] > 0
                else 0.0,
...
def main():
    ...
    print(
        f"   Traditional Unassessed/Rejected: {report['executive_summary']['traditional_rejected_or_unassessed_count']}"
    )
    print(
        f"   Vyapar Pulse Included: {report['executive_summary']['vyapar_pulse_included_count']} ({report['executive_summary']['inclusion_rate_pct']}%)"
    )
    print(
        f"   Total Credit Unlocked: INR {report['executive_summary']['total_credit_unlocked_inr']:,.2f}"
    )
